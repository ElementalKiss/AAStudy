...
* 입력 반복자(input iteraotr): 현재 위치에 있는 데이터에 대해 읽기만 가능한 반복자.
...
* 순반향 반복자(forward iteraotr): 입출력 모두 할 수 있지만, 한 위치에서만 되풀이해서 읽고 쓸 수 있다. 뒤로 이동할 수 없다.
...
* 복잡도(complexity)를 보장하고 있다.
* 이는 같은 문제에 대해 취할 수 있는 여러 접근 방법에 대한 정성적인 비교 도구.

## 시간 복잡도

* 상수 시간(constant time): n이 변하더라고 영향을 전혀 받지 않고 수행. 아주 빠른 시간 복잡도.
...
* 선형 시간(linear time): n의 증가에 따라 동일한 비율로 증가한다.
